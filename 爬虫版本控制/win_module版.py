...
def get_data(text):
    soup = BeautifulSoup(text.strip(),  "html5lib")
    # print(soup.prettify())
...
def _init():
    ...
    browser = webdriver.Chrome(r'D:/pachong/work/chromedriver.exe',options=chrome_options)
    return browser
...
def chrome_spyder(url):
    if "http"  in url or "https"  in url:
        return chrome_spider_son(url,10,30)
    else:
        url = "http://" + url
        return chrome_spider_son(url,10,30)
print(chrome_spyder("www.9041y.com"))
