...
@retry((requests.exceptions.RequestException), tries=2, delay=0.5, backoff=1,)
...
def selenium_or_not(url, html):
    ...
    if len(html)==0:
        return True
            ...
            or ("window.location.search" in script_1) \
            or ("console" and "app" in script_1) \
...
def chrome_spyder_01_plus(url, selenium_jsTimeout,timeout):
    ''' Args:
    ...
    browser.set_script_timeout(selenium_jsTimeout)
    try:
        browser.get(url)
...
@func_set_timeout(15)
def task(url,selenium_jsTimeout,timeout):
    return chrome_spyder_01_plus(url, selenium_jsTimeout,timeout)
def chrome_spyder_01(url,selenium_jsTimeout,timeout):
    try:
        return task(url,selenium_jsTimeout,timeout)
